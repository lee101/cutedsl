...
20–33% over raw Taylor and captures nearly all of the two-delta fit's gain.
...
Reference-aware oracle routing between learned and Taylor adds at most 0.38 dB
PSNR over the better fixed method in the existing image sweep. Routing is
therefore secondary to improving the forecaster. A full-resolution author
screen confirmed that all 36 learned/Taylor candidates retained the requested
subject or scene; all 18 skip controls were unusable. This is a failure-mode
screen, not a blinded aesthetic evaluation.
...
- [ ] Image and denoiser-only timing sweep for schedule-calibrated momentum
