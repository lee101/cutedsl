...
Advanced cache-procedure ablation on the same six held-out step-6→14 decodes:

| procedure | PSNR | SSIM |
| :--- | ---: | ---: |
| calibrated local momentum | 21.48 dB | 0.7731 |
| pruned 8-NN scalar residual | 21.84 dB | 0.7881 |
| variance-shrunk residual | 21.84 dB | **0.7883** |
| learned cache adapter | **21.92 dB** | 0.7877 |

The learned head has 7,312 parameters and is zero-initialized to the scalar
cache gate. On the 40-trajectory held-out fold it improves aggregate relL2 from
0.2320 (local) and 0.2250 (scalar cache gate) to 0.2225, and improves every
tested horizon. Training took 132 seconds on CPU and resident one-thread CPU
inference takes 3.28 ms per 16×64×64 latent. The RTX 5090 safety check found
less than 2.5 GiB allocatable VRAM under production load and deferred rather
than forcing the GPU run. The 31 KB weights and schedule coefficients are at
https://huggingface.co/lee101/zimage-latent-teleport-cache-adapter. The complete
five-column grid is `paper/figures/cache_procedure_grid.png`.
...
- [x] Variance-aware and learned cache-procedure image grid
- [x] Publish focused cache-adapter weights, schedule table, and metrics
