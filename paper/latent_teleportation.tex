...
\begin{enumerate}
  ...
  \item \textbf{Retrieve residuals, not complete moves.} Keep a prompt-similar
    pool in host memory, prune it with the observed motion descriptor, and
    prefetch only the retained $k$ residual records. The eight-neighbour gate is
    the best measured setting and stays below one millisecond in the resident
    CPU predictor benchmark.
...
end-to-end result is $\CorrectedSpeedupKThree\times$. This sets the systems
priority: overlap or fuse fixed work while moving the method to a longer
quality schedule. The immediate milestone is a held-out 30-step teacher experiment with
...
Confidence gating and LoRA-space transport remain distinct follow-on hypotheses.
