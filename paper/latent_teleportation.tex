...
held-out long-warp grid. The production helper
...
long warp, averaged over six prompts from fold 0. Metrics compare each decoded
forecast with the decoded recorded endpoint. Higher is better.}
\label{tab:knn-visual}
\end{table}

\begin{figure}[p]
\centering
\includegraphics[width=\textwidth,height=0.82\textheight,keepaspectratio]{figures/knn_visual_grid.png}
\caption{Held-out long-warp visual comparison. Every row is a test-fold prompt;
the retrieval column uses eight motion-pruned trajectories and the two-input
residual gate. Panels are direct VAE decodes of the recorded endpoint and two
forecasted endpoint latents, with no content editing.}
...
21.48 to 21.84\,dB PSNR and from 0.773 to 0.788 SSIM. These results support a
